...
# Training arguments
flags.DEFINE_integer('num_classes', 31, 'The number of classes to predict.')
...
flags.DEFINE_string('weighting', "MFB",
                    'Choice of Median Frequency Balancing or the custom ENet class weights.')  # ENET   MFB
...
def run():
    with tf.Graph().as_default() as graph:
        ...
        total_loss = tf.log(tf.clip_by_value(total_loss, 1e-8, tf.reduce_max(total_loss)))
